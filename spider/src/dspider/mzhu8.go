...
func downloadData(baseUrl string, downNum int, spiderName string, download *sync.WaitGroup, novelInfo chan dcrawl.NovelField, toMongo chan dcrawl.NovelField) {
	for i := 0; i < downNum; i ++ {
		...
		go func() {
			for {
				info, isClose := <-novelInfo
				if !isClose {
					dcrawl.Log.Infof("info 管道关闭进程退出")
					download.Done()
					return
				}
...
func Mzhu8Run(np *dcrawl.SpiderContent) {
	...
	/* 获取小说基本信息 */
	go downloadData(np.BaseUrl, downloadNum, np.SpiderName, &downloadGroup, novelData, saveData)

	/* 保存小说 */
	for iinfo := range saveData {
		saveGroup.Add(1)
		go saveToMongo(np.MI, np.SpiderName, &saveGroup, iinfo)
	}

	downloadGroup.Wait()
	dcrawl.Log.Info("mzhu8 所有书籍下载完毕!!!")
	close(saveData)
	saveGroup.Wait()
	dcrawl.Log.Info("mzhu8 所有书籍保存完毕!!!")
